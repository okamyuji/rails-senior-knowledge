...
- `Refinement#target` — Refinement が拡張しているクラス／モジュール
  - Ruby 3.2 で `Refinement#refined_class` として導入されたが、Refinement がクラスだけでなくモジュールも拡張できることを反映するため、Ruby 3.3 で `Refinement#target` に改名（`refined_class` は deprecated に）
  - Ruby 3.4 で `Refinement#refined_class` は削除されたため、現在は `target` のみ使用可能
...
2. レシーバのシングルトンクラス（およびそのprepend/include）
3. レシーバのクラスにprependされたモジュール
4. レシーバのクラス自身
5. レシーバのクラスにincludeされたモジュール
6. スーパークラス（同じ手順で辿る）
7. ... → BasicObject

```

トピック02「メソッド探索チェーン」と同様、`prepend`はクラス本体より**先**に挿入される点に注意してください。
