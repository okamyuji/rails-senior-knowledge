...
1. オブジェクトのシングルトンクラス
2. オブジェクトのクラスに`prepend`されたモジュール（クラス本体より**先**、新しいものが先・LIFO）
3. オブジェクトのクラス自身
4. オブジェクトのクラスに`include`されたモジュール（クラス本体の**後**、新しいものが先・LIFO）
5. スーパークラス（2〜4と同じ手順を繰り返す）
6. 最終的に`BasicObject`まで到達

`ancestors` 配列の並び順がそのまま探索順となります。`prepend` されたモジュールは ancestors 上でクラス自身よりも前に挿入されるため、最初に評価される点が重要です。
