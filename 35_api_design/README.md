...
class SessionsController < ApplicationController
  ...
  # `by:` `with:` ともに**コントローラインスタンスのコンテキストで評価**される
  # 引数なし lambda です。`request` などのコントローラメソッドに直接アクセスします
  # （`by: ->(req) { ... }` のように引数を取る形ではありません）。
  # 複数の rate_limit を宣言する場合は `name:` で区別が必要です。
  rate_limit to: 100, within: 1.hour, name: "api_key",
...
冪等性キーの実装では2つの落とし穴に注意します。

1. **キーの一意性**: `Idempotency-Key` ヘッダだけをキーにすると、別ユーザが同じキーを送信した場合に他人のレスポンスが返るなど深刻な問題が起きます。`user_id` + `request_method` + `request_path` + リクエストボディのダイジェストを組み合わせる必要があります。
2. **アトミックな claim**: cache.get → 処理 → cache.set のパターンは非アトミックで、並行リクエストが両方とも処理を実行してしまいます。Redis `SET NX` やDBユニーク制約で「最初に claim できたリクエストだけが処理を実行する」ようにします。

```ruby

# Idempotency-Keyヘッダをチェックするミドルウェア（Redis を claim ストアに利用）
class IdempotencyMiddleware
  ...
  def call(env)
    ...
    header_key = request.get_header('HTTP_IDEMPOTENCY_KEY')

    return @app.call(env) unless IDEMPOTENT_METHODS.include?(request.request_method)
    return @app.call(env) unless header_key

    # キーをユーザー / メソッド / パス / bodyダイジェストにバインドする
    body = request.body.read
    request.body.rewind
    body_digest = Digest::SHA256.hexdigest(body)
    user_id = env['warden']&.user&.id || 'anonymous'
    composite_key = [
      'idem', user_id, request.request_method, request.path, header_key, body_digest
    ].join(':')

    @redis_pool.with do |redis|
      # SET NX EX でアトミックに claim する。
      # 後続の同一リクエストはキャッシュ済みレスポンスを返す。
      claimed = redis.set(composite_key, '__PENDING__', ex: TTL, nx: true)

      if claimed
        status, headers, body_io = @app.call(env)
        body_array = []
        body_io.each { |chunk| body_array << chunk }
        body_io.close if body_io.respond_to?(:close)

        cached = JSON.generate(status: status, headers: headers, body: body_array.join)
        redis.set(composite_key, cached, ex: TTL)
