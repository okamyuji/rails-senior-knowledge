...
solid_queue_blocked_executions    ← `limits_concurrency` で待機させられているジョブ
...
[Enqueue]
    ...
    │
    ├─ ConcurrencyControls(`limits_concurrency`) のキー競合
    │    → blocked_executions に登録（concurrency_duration の期限付き）
    │         │
    │    先行ジョブ完了時の `BlockedExecution.unblock`、または
    │    Dispatcher の ConcurrencyMaintenance（concurrency_maintenance_interval
    │    ごとの TimerTask、デフォルト 600 秒）による期限切れ・解放可能判定
    │         │
    │    → ready_executions に移動します（promote_to_ready）
    │
    └─ 即時実行
...
class BillingJob < ApplicationJob
  # solid_queue 1.3 の `limits_concurrency` は `key:` が必須、`to:` の
  # デフォルトは1、`group:` でジョブ間でセマフォを共有でき、`duration:` で
  # ブロック期間の上限を指定、`on_conflict:` で `:block` / `:discard` を選べる
...
# solid_queue 1.3 公式 README の推奨パターン。
# 注意: `ENV.fetch("SOLID_QUEUE_IN_PUMA", false)` だと環境変数が "false" 等の
# 文字列でも真と評価されてしまうため、`ENV["SOLID_QUEUE_IN_PUMA"]` の存在チェックで判定する
