...
4. 文字列のSymbol変換: 静的に書かれた`:foo`のようなSymbol（immortal symbol）はGC対象外で常駐します。Ruby 2.2以降では`to_sym`で動的生成されたSymbol（mortal symbol）はGC対象になったため、ユーザー入力を`to_sym`しても以前のような無制限増加は起きませんが、ハッシュキーなどで強参照されたまま残ると依然リークの原因になります
